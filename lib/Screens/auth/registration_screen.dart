    ...
    try {
      ...
      if (content.contains('|')) {
        ...
        if (parts.length >= 2) {
          ...
          // roleSpecificUID may itself already be in "androidId|userId" form
          // (UID.txt gets overwritten with that combined format after the
          // first successful registration) — compare against just the
          // android-id portion so a second attempt on the same device still
          // matches instead of silently falling through.
          final bareAndroidId =
          androidId.contains('|') ? androidId.split('|').first.trim() : androidId;
