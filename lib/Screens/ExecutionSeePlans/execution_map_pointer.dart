...
//           print("⚠️ Skipping pointer with empty locateId");
//           continue;
//         }
//
...
//         if (isCaptured) {
//           capturedCount++;
//           print("⏭️ Skipping captured locateId: ${pointer.locateId}");
//         } else {
...
//       }
//     } else {
//       print("⚠️ No pointers passed to MapScreenpointer");
//
//       if (mounted) {
//         ScaffoldMessenger.of(context).showSnackBar(
//           const SnackBar(
...
//       final marker = Marker(
//         markerId: MarkerId(item.locateId.toString()),
//         position: LatLng(
//           item.latitude,
//           item.longitude,
//         ),
...
//
//         bool isNear = distanceInMeters <= 200;
//
//         return Dialog(
...
//                             : "Move within 200m to unlock Next",
//                         textAlign: TextAlign.center,
//                         style: TextStyle(
//                           fontSize: 13,
...
//
//                       Text(
//                         "Lat: ${pointerLat.toStringAsFixed(6)}",
//                         style: TextStyle(fontSize: 12),
//                       ),
//
//                       Text(
//                         "Lng: ${pointerLng.toStringAsFixed(6)}",
//                         style: TextStyle(fontSize: 12),
//                       ),
//
//                       SizedBox(height: 12),
...
//                                   locateId: pointerId, // <-- FIX: Use pointerId
//                                 ),
//                               ),
//                             );
//                           },
...
//     => marker.markerId.value != 'current_location' && marker.markerId.value != 'accuracy_circle'
//     ));
//
//     _markers.clear();
//     _circles.clear();
...
//                   CrashReportManager.storeLogMessage(
//                       "event=low_accuracy_warning_shown | "
//                           "planCode=${widget.planCode} | "
//                           "accuracy=${_currentAccuracy}m | "
//                           "threshold=100m | "
//                           "timestamp=${DateTime.now().toIso8601String()}"
...
//                             onPressed: () {
//                               CrashReportManager.storeLogMessage(
//                                   "event=improve_location_accuracy_selected | "
//                                       "planCode=${widget.planCode} | "
//                                       "timestamp=${DateTime.now().toIso8601String()}"
//                               );
//                               Navigator.of(context).pop();
...
//                             onPressed: () {
//                               CrashReportManager.storeLogMessage(
//                                   "event=continue_with_low_accuracy_selected | "
//                                       "planCode=${widget.planCode} | "
//                                       "accuracy=${_currentAccuracy}m | "
//                                       "timestamp=${DateTime.now().toIso8601String()}"
//                               );
...
//         _showPointersFromData(filteredPointers);
//
//         // Zoom to nearby pointers after location is fetched
//         _waitForLocationAndZoomToMarkers();
//       } else {
//         // All pointers captured
//         if (mounted) {
//           setState(() {
...
//   }
//
//   // ============ NEW: Wait for location and zoom to markers ============
//   void _waitForLocationAndZoomToMarkers() {
//     // Check if location is already fetched
//     if (_isLocationFetched && _currentPosition != null) {
...
//         timer.cancel();
//         // If location still not fetched, fit all markers
//         _fitAllMarkersToMap();
//         print(" Location timeout - fitting all markers");
//       }
//     });
//   }
//
//   // ============ NEW: Zoom to nearby markers (similar to ReworkMap) ============
//   Future<void> _zoomToNearbyMarkers() async {
//     if (_currentPosition == null || _pointerMarkers.isEmpty) {
//       print(" Cannot zoom: no location or no markers");
//       return;
//     }
//
//     print(" Zooming to nearby markers...");
//
//     // Define search radius in kilometers
//     double searchRadius = 5.0; // Start with 5km
//     List<Marker> nearbyMarkers = [];
//
//     // Find all pointer markers within the search radius
//     for (var marker in _pointerMarkers) {
//       double distance = Geolocator.distanceBetween(
//         _currentPosition!.latitude,
//         _currentPosition!.longitude,
...
//     }
//
//     // If no markers found, expand to 10km
//     if (nearbyMarkers.isEmpty) {
//       print(" No markers within 5km, expanding to 10km...");
//       searchRadius = 10.0;
//
//       for (var marker in _pointerMarkers) {
//         double distance = Geolocator.distanceBetween(
...
//     }
//
//     // If still no markers, show all markers
//     if (nearbyMarkers.isEmpty) {
//       print(" No markers within 10km, showing all markers");
//       _fitAllMarkersToMap();
//       return;
//     }
//
//     print(" Found ${nearbyMarkers.length} markers within ${searchRadius}km");
//
...
//     );
//
//     print(" Zoomed to ${nearbyMarkers.length} nearby markers");
//   }
//
//   // ============ NEW: Fit all markers to map ============
//   void _fitAllMarkersToMap() {
//     if (_pointerMarkers.isEmpty || _currentPosition == null) {
//       // If no markers, just move to current location
//       _moveCameraToCurrentLocation(animate: true);
//       return;
//     }
//
//     print(" Fitting all ${_pointerMarkers.length} markers to map");
//
//     double minLat = _currentPosition!.latitude;
//     double maxLat = _currentPosition!.latitude;
//     double minLng = _currentPosition!.longitude;
...
//       );
//       _markers.add(marker);
//       _pointerMarkers.add(marker); // Store for zoom functionality
//     }
//     setState(() {});
//   }
//
...
//                   if (artworkName != null && artworkName.isNotEmpty)
//                     Container(
//                       margin: EdgeInsets.only(top: 4), // Reduced from 6
//                       padding: EdgeInsets.symmetric(
//                         horizontal: 14,
...
//                             style: TextStyle(color: Colors.white, fontSize: 13), // Added fontSize
//                           ),
//                           style: ElevatedButton.styleFrom(
//                             backgroundColor: ColorsConst.primaryColor,
//                             padding: EdgeInsets.symmetric(vertical: 12), // Reduced from 14
...
//         await _logLocation("periodic_location_refresh", smoothedPosition);
//
//         // Re-zoom to nearby markers on location update
//         _zoomToNearbyMarkers();
//       }
//     } catch (e) {
//       CrashReportManager.storeLogMessage(
...
//       _getAddressFromLatLng(tappedPoint.latitude, tappedPoint.longitude, showToast: true);
//
//       // Zoom to nearby markers after manual selection
//       _zoomToNearbyMarkers();
//
//       Fluttertoast.showToast(
//         msg: S.of(context).locationResumeGPS,
...
//                           "timestamp=${DateTime.now().toIso8601String()}"
//                   );
//                   // Zoom to nearby markers after map is created
//                   _zoomToNearbyMarkers();
//                 },
//                 onTap: _onMapTapped,
//                 mapType: MapType.hybrid,
