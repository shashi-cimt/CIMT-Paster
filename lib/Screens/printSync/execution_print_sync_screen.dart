...
//     if (!mounted) {
//       // print(' loadSyncData called but widget not mounted');
//       return;
//     }
//
//     // print(' loadSyncData() started at ${DateTime.now()}');
//
//     setState(() {
//       isRefreshing = true;
//     });
...
//
//       // print(' Loaded ${metadata.length} items from Hive');
//
//       if (mounted) {
//         setState(() {
...
//
//         // print(' UI updated: ${metadata.length} total, ${filteredMetadata.length} filtered');
//       }
//     } catch (e) {
...
//           return metadata.VillageCode?.toLowerCase().contains(lowercaseQuery) == true ||
...
//
//       if (alreadySynced) {
//         print("✅ ALREADY SYNCED: $planId - ${metadata.PrintNo}");
//
//         // Delete from Hive
...
//       if (!syncingItems.contains(uniqueKey)) break;
//       _updateIndividualProgress(uniqueKey, i / 100.0);
//     }
//   }
...
//           print("⏭️ SKIPPING ALREADY SYNCED: $key");
//           // Delete from Hive
//           await ExecutionImageUploadHiveRepository().deleteMetadata(
//               metadata.ServerPlanId.toString(),
//               metadata.PrintNo.toString()
...
//       }
//
//       print("📊 Items to sync: ${itemsToSync.length} (out of ${loadedMetadata.length})");
//
...
//       // print('🔔 Background sync completed: $_previousBackgroundSyncCompleted -> $backgroundSyncCompleted');
//
...
//       // print('🔄 Background sync status: true -> false');
//
//       Future.delayed(Duration(milliseconds: 1000), () {
//         if (mounted) {
//           // print('🔄 Reloading after status change...');
//           loadSyncData();
//         }
//       });
//     }
...
class _PrintSyncScreenState extends ConsumerState<PrintSyncScreen> {
  ...
  void loadSyncData() async {
    ...
    if (!mounted) {
      print('⚠️ [loadSyncData] Widget not mounted, skipping load');
      return;
    }
    ...
    try {
      ...
      if (mounted) {
        ...
        print('⚠️ [loadSyncData] Widget not mounted after data load');
      }
    } catch (e) {
  ...
  void _syncPrint(String planId) async {
    ...
    if (isBackgroundSyncInProgress) {
      print('⚠️ [_syncPrint] Background sync in progress, cannot sync');
      Fluttertoast.showToast(
        msg: "Cannot sync: Background sync is in progress",
        toastLength: Toast.LENGTH_LONG,
        gravity: ToastGravity.CENTER,
    ...
    if (isPrintSyncInProgress || isSyncAllInProgress) {
      print('⚠️ [_syncPrint] Sync already in progress');
      Fluttertoast.showToast(
        msg: "Sync already in progress",
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.BOTTOM,
    ...
    try {
      ...
      if (success) {
        ...
        print('⚠️ [_syncPrint] Sync FAILED for $planId: $remarks');
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('❌ Failed to sync Print ${metadata.PrintNo}: $remarks'),
          backgroundColor: Colors.red,
          duration: Duration(seconds: 3),
  ...
  Future<void> _simulateProgress(String uniqueKey, int start, int end, Duration interval) async {
    ...
    for (int i = start + 1; i <= end; i++) {
      ...
      if (!syncingItems.contains(uniqueKey)) {
        print('⚠️ [_simulateProgress] Item removed from syncing list, stopping progress simulation');
        break;
      }
      _updateIndividualProgress(uniqueKey, i / 100.0);
    }
  ...
  void _syncAllPrints() async {
    ...
    if (isBackgroundSyncInProgress) {
      print('⚠️ [_syncAllPrints] Background sync in progress, cannot start');
      Fluttertoast.showToast(
        msg: "Cannot sync: Background sync is in progress. Please wait...",
        toastLength: Toast.LENGTH_LONG,
        gravity: ToastGravity.CENTER,
    ...
    if (isPrintSyncInProgress || isSyncAllInProgress) {
      print('⚠️ [_syncAllPrints] Sync already in progress');
      Fluttertoast.showToast(
        msg: "Sync already in progress",
        toastLength: Toast.LENGTH_SHORT,
        gravity: ToastGravity.BOTTOM,
    ...
    try {
      ...
      for (var metadata in loadedMetadata) {
        String key = "${metadata.ServerPlanId}_${metadata.PrintNo}";
        if (alreadySyncedKeys.contains(key)) {
          print("⏭️ [_syncAllPrints] SKIPPING ALREADY SYNCED: $key");
          // Delete from Hive
          await ExecutionImageUploadHiveRepository().deleteMetadata(
              metadata.ServerPlanId.toString(),
              metadata.PrintNo.toString()
          );
          print('🗑️ [_syncAllPrints] Deleted already synced metadata: $key');
        } else {
          itemsToSync.add(metadata);
      ...
      if (result is Map<String, dynamic>) {
        ...
        if (result.containsKey('results') && result['results'] is List) {
          ...
          print('⚠️ [_syncAllPrints] No results list in response, processing as batch');
          for (var metadata in itemsToSync) {
            String planId = metadata.ServerPlanId.toString();
            setState(() {
              syncStatusMap[planId] = SyncStatus(
      ...
      if (success) {
        ...
        print('⚠️ [_syncAllPrints] Sync all FAILED: $remarks');
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(
          content: Text('${S.of(context).failedSyncAllPlan}: $remarks'),
          backgroundColor: Colors.red,
          duration: Duration(seconds: 3),
