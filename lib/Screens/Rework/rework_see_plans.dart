...
import 'package:canimage/Screens/Rework/rework_map.dart';
...
import '../../Repository/rework_image_upload_repository.dart';
import '../../generated/l10n.dart';
import '../../utils/fonts.dart';
import '../../utils/textStyle.dart';

final currentScreenProvider = StateProvider<String>((ref) => 'rework');
final reworkOfflineCountRefreshProvider = StateProvider<int>((ref) => 0);

class ReworkScreen extends ConsumerStatefulWidget {
  final Function(String) changeLanguage;

  const ReworkScreen({super.key, required this.changeLanguage});
  @override
  _ReworkScreenState createState() => _ReworkScreenState();
}
...
class _ReworkScreenState extends ConsumerState<ReworkScreen> {
  ...
  Future<void> _refreshPlans() async {
    try {
      final uploadRepository = ReworkImageUploadHiveRepository();
      ...
      await ReworkCompletedUploadRepository().clearAll();
      print('Cleared completed uploads - fresh start');
      await ref.refresh(reloadReworkPlansProvider(true).future);
      await ReworkBalanceCountChangeRepository().clearAllOfflineCounts();

      ref.read(reworkOfflineCountRefreshProvider.notifier).state++;
      _offlineCountCache.clear();
      _processedGroupedPlans = null;

      final plansAsyncValue = ref.read(reworkPlansProvider);
  ...
  void _onGroupRowClick(List<ReworkModel> group) async {
  ...
  Future<int> _getCachedOfflineCount(String groupKey, ReworkModel firstPlan) async {
  ...
  Map<String, Map<String, dynamic>> _processPlansData(List<ReworkModel> plans) {
    final projectPlanGroups = <String, Map<String, dynamic>>{};

    for (var plan in plans) {
      // Assuming ReworkModel has a projectName field. If not, you can use planCode as the project identifier
    ...
    for (var entry in groupedPlans.entries) {
      ...
      if (planMatches) {
        ...
        for (var villageEntry in allVillageGroups.entries) {
          ...
          final groupPlans = villageData['plans'] as List<ReworkModel>;
                              ...
                              Expanded(
                                ...
                                child: Consumer(
                                  builder: (context, ref, child) {
                                    ref.watch(reworkOfflineCountRefreshProvider);
                                    final isRefreshing = ref.watch(isRefreshingProvider);

                                    return FutureBuilder<int>(
                                      key: ValueKey('${villageKey}_${ref.watch(reworkOfflineCountRefreshProvider)}_$isRefreshing'),
  ...
  Widget build(BuildContext context) {
    ...
    final plansState = ref.watch(reworkPlansProvider);
    final isRefreshing = ref.watch(isRefreshingProvider);
    final reworkOfflineCountRefresh = ref.watch(reworkOfflineCountRefreshProvider);
                  ...
                  child: plansState.when(
                    data: (plans) {
                      ...
                      return FutureBuilder<Map<String, Map<String, dynamic>>>(
                        key: ValueKey('${_processedGroupedPlans!.length}_${reworkOfflineCountRefresh}_${isRefreshing}_$_searchQuery'),
